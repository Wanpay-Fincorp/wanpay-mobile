    ...
    <SafeAreaView style={tw`flex-1 pt-4 pb-8 bg-[${DARK_BG}]`}>
      ...
      <View style={tw`flex-1 px-5`}>
        <View style={tw`flex-row items-start mt-8 mb-7`}>
          ...
          <View>
            <Text style={tw`text-gray-900 text-[20px] font-bold tracking-tight`}>Transaction history</Text>
            <Text style={tw`text-gray-400 text-[12px] mt-0.5`}>{transactions.length} transactions</Text>
          </View>
        </View>

        <RefreshableScrollView onRefresh={onRefresh} refreshing={refreshing} showsVerticalScrollIndicator={false} contentContainerStyle={tw`pb-24`}>
          {loading ? (
            ...
            transactions.map((txn) => (
              <View key={txn.id} style={tw`bg-gray-50 border border-gray-200 rounded-2xl mb-2.5 overflow-hidden`}>
